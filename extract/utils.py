...
class TermDataset:
    ...
    def __init__(self, dataset):
        ...
        for i in range(len(dataset)):
            term = dataset[i]['instruction'][18:-1].lower()
            self.termidx[term] = i
            self.terms.append(term)
        
    def __getitem__(self, idx):
        if isinstance(idx, int):
            return self.dataset[idx]['wiki']
        elif isinstance(idx, str):
            idx = idx.lower()
            return self.dataset[self.termidx[idx]]['wiki']
    ...
    def search(self, content):
        ...
        result = []
        for i, term in enumerate(self.termidx):
            if content in term:
                result.append((i, term))
        return result


def gen_prompt(wdata: TermDataset, query):
    from frame import frame
    try:
        content = wdata[query]
    except:
        raise ValueError()
    prompt = frame.format(wiki=content)
